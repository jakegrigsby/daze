...
def ContractiveAutoEncoder(encoder, decoder, gamma, preprocessing_steps=[]):
    loss_funcs = [loss.reconstruction(), loss.contractive(.1)]
...
def DenoisingAutoEncoder(encoder, decoder, gamma, preprocessing_steps=[]):
    ...
    return Model(encoder, decoder, preprocessing_steps=preprocessing_steps, loss_funcs=loss_funcs)

def VariationalAutoEncoder(encoder, decoder, preprocessing_steps=[]):
    loss_funcs = [loss.reconstruction(), loss.kl(1.)]
    return Model(encoder, decoder, preprocessing_steps=preprocessing_steps, loss_funcs=loss_funcs, forward_pass=forward_pass.probabalistic_encode_decode)

def BetaVariationalAutoEncoder(encoder, decoder, beta, preprocessing_steps=[]):
    loss_funcs = [loss.reconstruction(), loss.kl(beta)]
    return Model(encoder, decoder, preprocessing_steps=preprocessing_steps, forward_pass=forward_pass.probabalistic_encode_decode, loss_funcs=loss_funcs)
